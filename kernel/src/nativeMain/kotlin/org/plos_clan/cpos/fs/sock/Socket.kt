...
internal abstract class AbstractSocket(
    ...
    fun setReceiveTimestamp(enabled: Boolean): VfsResult<Unit> =
        updateOptions { copy(receiveTimestamp = enabled) }
    ...
    protected fun captureReceiveTimestampLocked(): ULong? =
        if (options.receiveTimestamp) TscClock.nanoTime() else null
