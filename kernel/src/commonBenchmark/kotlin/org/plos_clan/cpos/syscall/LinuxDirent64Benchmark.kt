...
class LinuxDirent64Benchmark {
    ...
    private lateinit var entries: List<LinuxDirent64>
    ...
    fun prepare() {
        ...
        entries = List(entryCount) { index ->
            LinuxDirent64(name, InodeId(index.toULong()), InodeType.REGULAR, index.toLong() + 1L)
        }
        outputSize = entries.first().recordSize * entryCount
    }
