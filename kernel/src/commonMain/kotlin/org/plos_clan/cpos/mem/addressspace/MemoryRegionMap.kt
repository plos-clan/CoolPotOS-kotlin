...
import org.plos_clan.cpos.utils.isAligned
...
internal class MemoryRegionMap(
    ...
    private val pageSize: ULong,
    private val owner: MemoryRegionOwner? = null,
) : Iterable<MemoryRegion> {
    init {
        require(pageSize > 0uL && allocationStart <= allocationEnd && allocationEnd <= addressLimit)
    }
    ...
    private fun valid(region: MemoryRegion): Boolean =
        region.start < region.end &&
            region.start.isAligned(pageSize) &&
            region.end.isAligned(pageSize) &&
